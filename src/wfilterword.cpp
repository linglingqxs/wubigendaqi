...
WfilterWord::WfilterWord(QWidget*parent) : QWidget(parent), ui(new Ui::WfilterWord) {
    ui->setupUi(this);
...
void WfilterWord::on_pushButton_send_clicked() {
    ...
    QStringList tempStrList = fileContents.split(ui->lineEdit_fenge->text(), Qt::SkipEmptyParts);
...
void WfilterWord::readIni() {
    ...
    in >> this->fileList >> inputFenge >> inputWordNum >> inputTime >> isShunxu >> isSuiji >> isLimit;
...
void WfilterWord::saveIni() {
    ...
    out << this->fileList << ui->lineEdit_fenge->text() << ui->lineEdit_wordnum->text() << 0 // lineEdit_time.text
        << ui->radioButton_order_shunxu->isChecked() << ui->radioButton_order_suiji->isChecked() << ui->checkBox_dingshi->isChecked();
    file.close();
}
